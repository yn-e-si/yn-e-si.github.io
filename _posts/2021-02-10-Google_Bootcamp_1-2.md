...
이 image가 실제 프로그램상에서는 X와 같이 표기된 형태로 들어간다고 생각하면 된다.

이렇게 64 * 64 * 3 인 차원을 12288로 바꾸는 것을 '*Flatten한다* ' 라고하는데

바뀐 값이 해당 image의 차원의 수이다.

또한 이 차원 그대로 input이 되는 것이고 Binary Classification이라고 생각한다면 {0,1} 중에

하나의 값만을 가지게 된다.

![](https://yn-e-si.com/assets/images/1-2/2.JPG){: .align-center}

(x,y)는 input 과 output의 순서쌍이다.
...
$$
training \ sample의 \ 수 = n_x
$$

라고 표기하기로 약속한다.

- X: **n개의 차원을 갖는 m개의 데이터**

- Y: **1개의 차원을 갖는 m개의 데이터**

![](https://yn-e-si.com/assets/images/1-2/3.JPG){: .align-center}

위의 사진은 이 후 강의에서 쓰이게 될 용어에 대해 정리한 것

### 다음은 Logistic Regression에 대한 설명이다.

![](https://yn-e-si.com/assets/images/1-2/4.JPG){: .align-center}

우선, **Logistic Regression**은 **Binary classfication**에서 쓰이는 알고리즘이다.
...
$$
output: \hat{y} \ (0 \le \hat{y} \le 1)  \to P(y=1 |x)
$$



- *Logistic Regression*은 0~1사이에 값을 가지게 하기엔 확률상으로 어려움이 많음 -> 좋은 알고리즘은 X

- *Logistic Regression*은 *sigmoid함수*를 이용, 위에 그래프와 같은 형태



마지막으로, *Logistic Regression*은 w 와 a 로 이루어진 파라미터 Z 에 대하여
$$
\hat{y} = \sigma(w^tx + b)
$$
...
이는 Z값이 커지면 커질수록 1에 가까워지고, Z값이 작으면 작아질수록 0에 가까워진다.

**즉, Logistic Regression을 사용할 때의 핵심:**
$$
parameter- w,b를 \ 조정하여\ \hat{y}이 \ y=1이\ 되는\ 확률을\ 잘 \ 추정한 \ 수치가 \ 나올 \ 수 \ 있도록\ 하는\ 것
$$




### Logistic Regression의 parameter - w와 b를 정의하기 위한 Cost function에 대한 내용

![](https://yn-e-si.com/assets/images/1-2/5.JPG){: .align-center}
$$
i:\ i^{th} examples \\ L(Loss \ function): true\ label\ y를\ 갖을\ 때,\ 얼마나 \ 정확히\ \hat{y}을 \ 산출하는지
$$

- 제곱을 사용하여 L을 정의할 경우 *gradient descent*가 잘 나오지 않는다는 단점이 있기에 제곱으로 사용하지 않는다.
- *gradient descent*가 잘 나오지 않는다면 *optimization*하는데 어려움이 생기게 된다.
...
$$
1. \ y=1\ 일 \ 때,\ L(\hat{y},y)=-log\hat{y} \ \\ 따라서, \ 이 \ 값을 \ 최소로\ 만들기\ 위해선\ \hat{y}를\ 크게\ 만들어야\ 한다.\\ \\  \ \\ 2. \ y=0\ 일\ 때,\ L(\hat{y},y) = - log(1-\hat{y}) \\ 따라서,\ 이\ 값을\ 최소로\ 만들기\ 위해선\ \hat{y}를\ 작게\ 만들어야\ 한다.
$$
...
- 최적의 *w,b* 를 찾고, *J*를 줄이는 것이  *cost function*의 궁극적인 목표

### Gradient Descent

- *J(w,b)*를 최소로 갖는 *w,b*를 찾기 위해선 우선 *w,b*를 초기화 시켜줘야 함
- 일반적으로 *0*으로 초기화하거나 *random*으로 초기화 시킴

![](https://yn-e-si.com/assets/images/1-2/6.JPG){: .align-center}
$$
learning \ rate(= \alpha): 한번에 \ 얼마만큼 \ gradient\  descent를 \ 진행할지 \ 조절해주는 \ 역할  \\ \ derivative: w,b에 \ 얼마나\ 변화를\ 줄지\ 알려주는\ 값\ (\ 특정\ 지점에서의\ 함수의\ 기울기\ )
$$
...
- *Computation graph*는 파란선과 같이 왼쪽에서 오른쪽으로 계산해가는 과정이다.
- 이와 반대로 오른쪽에서 왼쪽으로 가는 빨간색 과정은 *derivative* 이다.
- *J*는 우리가 최소로 만들고 싶은 *cost function* 이다.

![](https://yn-e-si.com/assets/images/1-2/9.JPG){: .align-center}
$$
\frac{dJ}{dv} = dv = 3 \to  v가\ 변할\ 때,\ J에\ 미치는\ 영향을\ 말함
$$

- 즉 v가 변할 때 J는 3배만큼 커진다는 것을 의미

$$
\frac{dJ}{da} = \frac{dJ}{dv} *\frac{dv}{da} = 3 * 1 = 3 \to a가\ 변할\ 때,\ J에\ 미치는\ 영향을\ 말함
$$

- *a*값이 변하게 되면 *v*값이 변하게 되고 *v*값이 변하면 *J*값이 변하게 됨
- 즉 *chain rule*에 의해 *a*가 *J*에 미치는 영향을 구할 수 있음

### 표기법:

$$
\frac{dFinalOutputVar}{dvar} → dvar : 최종\ output에\ 관한\ derivative의\ 변수명 \\ \frac{dJ}{dv}  → dv : dv에\ 관한\ derivative의\ 변수명 \\ \frac{dJ}{du}→ du : du에\ 관한\ derivative의\ 변수명 \\  \frac{dJ}{da}→ da : da에\ 관한\ derivative의\ 변수명 \\ \frac{dJ}{db}→ db : db에\ 관한\ derivative의\ 변수명 \\ \frac{dJ}{dc}→ dc : dc에\ 관한\ derivative의\ 변수명
$$
...
-  **cost function의 값**을 계산할 때는, *left to right*인 **forward** 방식으로 계산

- **derivateives**를 계산할 때는, *right to left*인 **backwards** 방식으로 계산
...
앞서 언급했던 *Logistic regression*에 대하여 *Computation Graph*를 그려보면 위와 같음.
...
\frac{dL}{db} = dz → db
$$
이를 통해 *L(a,y)*는 1개의 example에 대해 다음과 같이 *update*가 된다.
...
- *m* 개의 *examples*에 대하여 적용할 때는 위와 같이 *전체 값의 평균* 을 구하면 됨
...
2. ***m examples* 에 대하여 반복한 뒤 평균을 구해줌**
   $$
...
- *딥러닝 알고리즘*을 적용할 경우, **Vectorized**를 이용한 것이 더 빠르게 결과값을 얻어 낼 수 있음

- *for-loop*을 사용하는 경우 속도가 현저히 느려지기 때문에, *딥러닝 알고리즘*에선 되도록 사용을 피해야 함

- *np.function*과 같은 **built-in** 기능을 사용하면, python이 paralleslism을 활용할 수 있게 계산을 하게 되어 속도가 더 빨라짐
...
$$
z^{(1)} = w^Tx^{(1)} + b,\ z^{(2)} = w^Tx^{(2)} + b,\ ... ,\ z^{(m)} = w^Tx^{(m)} + b 로\ 되어\ 있는\ 것을\ vector의\ 형태로\ 표현 \\ 즉,\ \ Z=[\ z^{(1)}\ \ z^{(2)}\ \ ... \ \ z^{(m)} \ ] = [\ (w^Tx^{(1)}+b) \ \ (w^Tx^{(2)}+b)\ \ \ ... \ \  (w^Tx^{(m)}+b) \ ] : \ (1*m)의\ 형태를\ 갖는  \  matrix\ 형태 \\
...
$$
dz^{(1)} = a^{(1)} - y^{(1)}, \ a^{(2)} - y^{(2)}, \ ..., \ a^{(m)} - y^{(m)}로\ 되어\ 있는\ 것을\ vector의\ 형태로\ 표현 \\
즉,\  dZ = [ \ dz^{(1)}\ dz^{(2)}\ ... \ dz^{(m)}\ ]  = [ \ a^{(1)}-y^{(1)}\ \ a^{(2)}-y^{(2)}\ \ ... \ \ a^{(m)}-y^{(m)} \ ]  \to \ (1*m)의\ 형태를\ 갖는\ matrix\ 형태 \\
...
- 좌측에 보이는 코드는 *for-loop*을 2번 사용하는 비 효율적인 *non-vectorized* 형식

  

이를 위의 **vectorization**을 사용하여 표기하면 다음과 같아짐
$$
...
따라서 *w,b*는 각각 한번의 *gradient descent*를 진행하면 다음과 같이 *update*가 됨
$$
w\ :=\ w-\alpha * dw \\
b\ :=\ b - \alpha * db
$$
...
→ 두 배열 간의 연산에서 최소한 하나의 배열의 차원이 1이라면(0번 축 or  1번 축 or 1행 or  1열) 가능
